...
{
	Handle hGameConf = LoadGameConfigFile("parachute.gamedata");
	if (hGameConf == INVALID_HANDLE)
	{
		SetFailState("gamedata/\"parachute.gamedata.txt\" not found");
	}
...
stock bool LookupAttachment(int client, char[] point)
{
    if (g_hLookupAttachment==INVALID_HANDLE){ CPrintToChat(client,"[ERROR] INVALID_HANDLE"); return false; }
...
void OpenParachute(int client)
{
	...
	if(g_bCvarModel){ // code taken from sm_hats 
		if(!LookupAttachment(client, "primary")){ CPrintToChat(client,"[ERROR] Attachment"); return; } // TODO : LookupBone offset of ValveBiped.Bip01_Head1 or other magic...
		...
		if(GetClientTeam(client) == 3)
		{
			...
			or[0] += fRight[0]*g_fCTfPos[0] + fForward[0]*g_fCTfPos[1] + fUp[0]*g_fCTfPos[2];
			or[1] += fRight[1]*g_fCTfPos[0] + fForward[1]*g_fCTfPos[1] + fUp[1]*g_fCTfPos[2];
			or[2] += fRight[2]*g_fCTfPos[0] + fForward[2]*g_fCTfPos[1] + fUp[2]*g_fCTfPos[2];

			DispatchKeyValue(ent, "model", g_sCT_Model);
		}
		else
		{
			ang[0] += g_fTfAng[0];
			...
			or[0] += fRight[0]*g_fTfPos[0] + fForward[0]*g_fTfPos[1] + fUp[0]*g_fTfPos[2];
			or[1] += fRight[1]*g_fTfPos[0] + fForward[1]*g_fTfPos[1] + fUp[1]*g_fTfPos[2];
			or[2] += fRight[2]*g_fTfPos[0] + fForward[2]*g_fTfPos[1] + fUp[2]*g_fTfPos[2];

			DispatchKeyValue(ent, "model", g_sT_Model);
		}
...
void CloseParachute(int client)
{
	if(g_bHasModel[client] && IsValidEntity(g_iParachute_Ent[client])){
		AcceptEntityInput(g_iParachute_Ent[client], "Kill");
